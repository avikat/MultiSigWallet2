...
contract MultiSigWallet {
    ...
    modifier notApproved(uint _txId) {
        require(!approved[_txId][msg.sender], "tx already pproved");
        _;
    }
    ...
    constructor(address[] memory _owners, address _admin) {
        required = uint((6*owners.length)/10) +1;
    ...
    function addOwner(uint _txId, address _owner) external onlyAdmin txExists(_txId)
    {
    ...
    function transfer(address _admin) external onlyAdmin 
    {
        isAdmin[_admin] = true;
